...
/*
  ...
  Renders a fixed, full-viewport canvas behind content (pointer-events: none),
  so the UI stays clickable while the smoke reacts to the cursor.
*/
...
export default function AuroraField({
  ...
  const cfg = useRef<Required<Omit<AuroraFieldProps, "className" | "style">>>({
    heatColor,
    heatIntensity,
    speed,
    scale,
  ...
  useEffect(() => {
    ...
    if (!gl) {
      console.error("AuroraField: WebGL unavailable");
      return;
    }

    // Respect reduced-motion: freeze drift + cursor reactions.
    ...
    function onMove(e: PointerEvent) {
      if (!cfg.current.interactive || reduce) return;
    ...
    function frame() {
      ...
      gl!.uniform1f(U.uSpeed, reduce ? 0 : c.speed);
